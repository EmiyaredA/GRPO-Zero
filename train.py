...
def main(config_path: str):
    ...
    for step, batch in enumerate(train_dataloader, start=1):
        ...
        reward = [episode.reward for episode in episodes] # 总奖励
        formatted_reward = [
            episode.reward_info["format_reward"] for episode in episodes # 格式奖励
        ]
        answer_reward = [episode.reward_info["answer_reward"] for episode in episodes] # 答案正确性
        num_finished_episodes = sum(episode.is_finished for episode in episodes) # 完成轨迹数
        mean_reward = np.mean(reward) # 平均奖励
        std_reward = np.std(reward) # 奖励标准差
        success_rate = np.mean(answer_reward) # 答案正确率
        format_reward = np.mean(formatted_reward) # 格式奖励
        grad_norm = results["grad_norm"] # 梯度范数
        entropy = results["entropy"] # 策略熵
        lr = optimizer.param_groups[0]["lr"] # 学习率
        loss = results["loss"] # 损失
